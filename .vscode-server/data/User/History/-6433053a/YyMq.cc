...
{
  uint64_t adjusted_window_size = window_size == 0 ? 1 : window_size;
  while ( adjusted_window_size > sequence_numbers_in_flight() ) {
    TCPSenderMessage segment = construct_segment( outbound_stream, adjusted_window_size );
...
TCPSenderMessage TCPSender::construct_segment( Reader& outbound_stream, uint64_t adjusted_window_size )
{
  string segment_payload;
  bool syn = next_seqno == 0;
  read( outbound_stream,
        min( adjusted_window_size - sequence_numbers_in_flight() - syn, TCPConfig::MAX_PAYLOAD_SIZE ),
        segment_payload );
...
void TCPSender::receive( const TCPReceiverMessage& msg )
{
  ...
  uint64_t msg_ackno_unwrapped = msg.ackno->unwrap( isn_, next_seqno );
  if ( msg_ackno_unwrapped > ackno && msg_ackno_unwrapped <= next_seqno ) {
    ackno = msg_ackno_unwrapped;
    clear_acked_segments( msg_ackno_unwrapped );
    handle_timer_on_receive();
  }
}

void TCPSender::clear_acked_segments( uint64_t msg_ackno_unwrapped )
{
  while ( !in_flight_messages.empty()
          && msg_ackno_unwrapped >= in_flight_messages[0].seqno.unwrap( isn_, next_seqno )
                                      + in_flight_messages[0].sequence_length() ) {
    in_flight_messages.erase( in_flight_messages.begin() );
  };
}

void TCPSender::handle_timer_on_receive()
{
