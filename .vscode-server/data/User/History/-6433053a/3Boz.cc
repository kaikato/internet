...
TCPSenderMessage TCPSender::constructSegment( Reader& outbound_stream, uint64_t adjusted_window_size )
{
...
void TCPSender::push( Reader& outbound_stream )
{
  ...
  while ( adjusted_window_size > sequence_numbers_in_flight() ) {
    TCPSenderMessage segment = constructSegment(
      Reader & outbound_stream, uint64_t adjusted_window_size ) if ( segment.sequence_length() == 0 )
    {
      return;
    }
