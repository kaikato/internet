...
class Router
{
  ...
  std::vector<Router::route> routing_table {};
...
private:
  // Finds the route in the routing table that longest prefix match to the destination IP in the datagram.
  struct route find_longest_match( InternetDatagram& dgram );
  // Sends the datagram along the route that is the best match, either to the next hop or to the destination itself.
  void send_dgram_on_route( InternetDatagram& dgram, struct route matched_route );
};
